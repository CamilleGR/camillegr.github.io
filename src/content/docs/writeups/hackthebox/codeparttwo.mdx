...
<center>
    <img src="cpt_illu.png" width="150px"/>
</center>
...
Sur le port 8000, nous trouvons une application web dont on peut télécharger le code source. Également il est possible de créer un compte et d'exécuter du code javascript.
...
En obtenant un shell, nous ne sommes pas connectés comme un utilisateur connu. Nous allons donc essayer de nous connecter avec un autre utilisateur. En énumérant les fichiers de l'application nous trouvons une base SQLite3. En listant directement les strings de cette base nous obtenons des hash de mot de passe, notamment au format md5 :
...
- Nous avons besoin d’une nouvelle configuration de backup : on copie l’existante et on modifie le dossier de backup (et la taille minimale du backup pour éviter les erreurs)
