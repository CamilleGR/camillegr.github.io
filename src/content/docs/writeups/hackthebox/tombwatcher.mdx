...
Le groupe infrastructure (et donc maintenant, Alfred) peut lire le mot de passe GMSA d'ansible dev. Nous pouvons donc récupérer le hash NTLM de ansible_dev$ pour se connecter avec ce compte.
...
Avec l'utilisateur Sam, nous allons changer l'owner de john (pour devenir l'owner), puis nous attribuer tous les droits sur l'utilisateur, avant de changer son mot de passe :
...
Nous allons donc restaurer cet utilisateur, changer son mot de passe, et générer un certificat d'authentification Administrator (ESC15) :
...
- Utiliser le certificat Administrator pour récupérer le hash de l'utilisateur et se connecter avec lui 
