...
- Une escalade de privilège via l'exploitation d'une librairie python vulnérable, nous permettant de faire du librairie Hijacking. 
...
La version de WING FTP est la version 7.4.3. Cette version est sujette à une vulnérabilité CVE-2025-47812 qui permet à un attaquant d’exécuter arbitrairement du code sans être authentifié. On va alors forger un reverse shell avec msfvenom, pousser le serveur à le télécharger et à l’exécuter : 
...
On télécharge notre archive et on lance l’exécutable avec les droits root pour créer notre re.py là où se trouve l’exécutable sur lequel on a les droits sudo. De cette manière, python va le charger et l’exécuter, en nous donnant un shell bash : 
